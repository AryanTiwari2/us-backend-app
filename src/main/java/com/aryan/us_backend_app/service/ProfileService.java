...
public class ProfileService {
    ...
    public UserRoomUpdateResponse addUserRoom(String username, String[] roomName, String token) throws Exception {
        ...
        if (userTypeStr == null || (!userTypeStr.equals(Constants.UserType.ADMIN.toString())
                && !userTypeStr.equals(Constants.UserType.MODERATOR.toString())))
        ...
        List<RoomModel> rooms = userOperation.getAllUsersRoom(userUpdated.userId);
...
List<RoomModel> rooms = userOperation.getAllUsersRoom(userUpdated.userId);
    ...
    public GetAllRoomResponse getAllRoomOfUser(String token) throws Exception {
        ...
        UserModel user = userOperation.getUserByName(username);
        List<RoomModel> rooms = userOperation.getAllUsersRoom(user.userId);
        return new GetAllRoomResponse(rooms);
    }

    public ChangeUserRoleResponse updateUserRole(ChangeUserRoleDto request, String token) throws Exception {
        Claims claims = JwtUtil.parseToken(token);
        String userTypeStr = (String) claims.get("userType");

        if (userTypeStr == null || !userTypeStr.equals(Constants.UserType.ADMIN.toString()))
            throw new Exception("Only admin can update user role");

        UserModel user = userOperation.getUserByName(request.username);
        if (user == null)
            throw new Exception("User does not exist");
        ...
        try {
            userType = Constants.UserType.valueOf(request.userType.toString());
        ...
        UserModel userUpdated = userOperation.saveUser(user);
        return new ChangeUserRoleResponse(userUpdated.username, userUpdated.userType, (String) claims.get("username"));
    }
}
